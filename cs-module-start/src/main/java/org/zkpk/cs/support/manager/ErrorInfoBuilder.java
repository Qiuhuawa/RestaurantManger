...
import org.zkpk.cs.common.result.ErrorInfo;
...
public class ErrorInfoBuilder implements HandlerExceptionResolver, Ordered {
     ...
     * 错误KEY
     */
    private final static String ERROR_NAME = "cs.error";
    ...
    public ErrorInfo getErrorInfo(HttpServletRequest request, Throwable error) {
        ...
        if (error instanceof LabException) {
        	LabException labException = (LabException) error;
        	errorInfo.setCode(labException.getCode());
        	errorInfo.setMsg(labException.getMsg());
        }
