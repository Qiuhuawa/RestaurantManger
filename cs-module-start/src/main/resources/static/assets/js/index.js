...
$(window).on("load", function () {
    ...
    var forTree = function (o) {
        for (var i = 0; i < o.length; i++) {
            ...
            try {
                if (!o[i]["path"]) {
                    urlstr = "<div><span><i class='drop_icon drop_right'></i><i class='" + o[i]["icon"] + "'></i>&nbsp;&nbsp;" + o[i]["name"] + "</span><ul>";
                } else {
                    urlstr = "<div><span name=" + o[i]["path"] + " onclick='loadMain(this);'><i class='" + o[i]["icon"] + "'></i>&nbsp;&nbsp;" + o[i]["name"] + "</span><ul>";
                }
                str += urlstr;
    ...
    var menuTree = function () {
        ...
        $navigation.find("div span").click(function () {
        	if ($(this).find('i:eq(0)').hasClass('drop_down')) {
        		$(this).find('i:eq(0)').removeClass('drop_down');
        		$(this).find('i:eq(0)').addClass('drop_right');
        	} else {
        		if (typeof($(this).attr('name'))== 'undefined') {
        			$(this).find('i:eq(0)').removeClass('drop_right');
        			$(this).find('i:eq(0)').addClass('drop_down');
        			$(this).parent().siblings().find('i:eq(0)').removeClass('drop_down');
        			$(this).parent().siblings().find('i:eq(0)').addClass('drop_right');
        		}
        	}
    ...
    $.post(ctx + "/res/getusermenubyuserid", {}, function (result) {
        if (result.meta.code === 1000) {
            var data = result.data.menu;
            var $crollbarInner = $(".scrollbar-inner");
            document.getElementById("navigation").innerHTML = forTree(data);
            menuTree();
    ...
    $("body").on("change", ".theme-switch input:radio", function () {
        var theme = $(this).val();
        $("body").attr("data-ma-theme", theme);
//        $.get(ctx + "/infoset/theme", {"theme": theme}, function (result) {
//            if (result.meta.code === 1000) {
//            	$MB.n_success("主题更换成功，下次登录时生效！");
//            }
//        });
    });
    // 修改个人信息
    ...
    $("#user__profile").on('click', function () {
        $.post(ctx + "/infoset/userinfo", function (result) {
            $breadcrumb.html("").append('<li class="breadcrumb-item">个人信息</li>');
            $main_content.html("").append(result);
        });
    });
});
