$(function () {
    ...
    $("#user-upload .btn-save").click(function () {
        ...
        if (name === "uplad") {
    	    ...
    	    $('#user-upload-button').show();//显示上传按钮
    	    $('#user-reupload-button').hide();//隐藏重新上传按钮
    	    $('#progress_div').hide();
 				...
 				if (data.meta.code == 1000) {
 					...
 					if(resultDate.FLAG){
 						if(passedCount == rowCount){
 							...
 							$('.step_one').hide();
 							$('#user-upload-button').hide();//隐藏上传按钮
 							$('#user-reupload-button').show();//显示重新上传按钮
 							$('.upload_name').html('上传excel文件');
 							...
 							if(0 != offedCount){
 								...
 								for(var i = 0; i < offedData.length; i++){
 									...
 									for(var j = 0; j < cells.length; j++){
 										...
 										switch(cell.key){
 										case 'loginName':
 											td = $(tr).find('td[class="loginName"]').text(cellValue);
 											break;
 										case 'realName':
 											td = $(tr).find('td[class="realName"]').text(cellValue);
 											break;
 										case 'note':
 										...
 										if(null != extraInfo && '' != extraInfo && 'undefined' != extraInfo){
 											if('realName' == cell.key){
 												td.attr('class','bggray');
 											}
 											if('loginName' == cell.key && '' == cellValue){
 												td.attr('class','bggray');
 											}
    ...
    $("#user-upload .btn-close").click(function () {
        closeUploadModal();
    });
});
...
function createTd(flag){
	...
	if(flag == '0'){
    	...
    	tdObj.className = "loginName";
    }else if(flag == '2'){
    	tdObj.className = "realName";
    }else if(flag == '3'){
...
function closeUploadModal() {
    $MB.closeAndRestModal("user-upload");
